...
\item Removed intrinsic sensor offset from the data automatically
\item Automatically identified each process condition using feature recognition on the data
\item Calculated the mean value at each process step for each sensor
\item Saved the calibrated results for each process step
\item This software reduced the time required to parse one data file from 1-2 hours down to 5 minutes
\end{itemize}
\end{rWorkProject}
...
\item Performed PCB diagnostic work, failure analysis, rework of SMD and through hole components.
\end{rSubsection}
